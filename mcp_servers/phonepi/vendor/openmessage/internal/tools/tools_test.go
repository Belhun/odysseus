...
func TestSendMessageMissingPhone(t *testing.T) {
	a := testApp(t)
...
func TestNormalizePhoneNumber(t *testing.T) {
	cases := map[string]string{
		"7605868615":    "+17605868615",
		"17605868615":   "+17605868615",
		"+17605868615":  "+17605868615",
		" (760) 586-8615": "+17605868615",
	}
...
func TestBuildSendPayloadHasSIM(t *testing.T) {
	sim := &gmproto.SIMPayload{SIMNumber: 1}
	payload := buildSendPayload("conv-1", "hi", "me", sim)
	if payload.GetSIMPayload() != sim {
		t.Error("expected SIM payload on send request")
	}
	if payload.GetMessagePayload().GetTmpID() == "" {
		t.Error("expected tmp ID on message payload")
	}
}
