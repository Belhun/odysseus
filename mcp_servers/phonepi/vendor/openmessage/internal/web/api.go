...
func APIHandlerFull(store *db.Store, cli *client.Client, logger zerolog.Logger, mcpHandler http.Handler, isConnected StatusChecker, unpair UnpairFunc, onDeepBackfill ...func()) http.Handler {
	return APIHandlerFullWithApp(nil, store, cli, logger, mcpHandler, isConnected, unpair, onDeepBackfill...)
}

func APIHandlerFullWithApp(toolApp *app.App, store *db.Store, cli *client.Client, logger zerolog.Logger, mcpHandler http.Handler, isConnected StatusChecker, unpair UnpairFunc, onDeepBackfill ...func()) http.Handler {
	mux := http.NewServeMux()

	if toolApp != nil {
		RegisterToolBridge(mux, toolApp)
	}
