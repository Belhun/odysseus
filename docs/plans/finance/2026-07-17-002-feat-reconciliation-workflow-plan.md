...
**Origin:** HomeBank `HbTxnStatus` / account `rdate`+`bal_recon` + docs/research/finance `features/multi-account-reconciliation.md`  
**Product Contract preservation:** N/A (bootstrap from port brief + docs/research/finance)
...
- HomeBank already proves a small status model + lock preference is enough for trust; Odysseus should add **sessions** (docs/research/finance / Quicken-style) so the agent has a structured object to drive, not a free-form register toggle like HomeBank’s manual doc workflow.
...
Research docs already specify the target shape: `finance_reconcile_sessions`, status `pending_review | cleared | reconciled`, session APIs, lock on finish (`docs/research/finance/features/multi-account-reconciliation.md`). Align HomeBank’s `none`/`void` with that doc by treating `none` ≈ uncleared register and `pending_review` as import-queue alias (optional alias, not a fifth lifecycle for MVP).
...
Model (from docs/research/finance, refined):
...
Extend `manage_finance` (do not invent a second finance tool). Follow aggregate-first + confirmation-gate patterns (`docs/research/finance/security/financial-data-security.md`, `integrations/finance/confirmation_gate.py`).
...
| **Polish** | Full wizard UX, session history UI, auto-suggest by amount/date, print report, CC copy | docs/research/finance polish |
...
docs/research/finance/features/multi-account-reconciliation.md  # keep in sync
...
| | |
|---|---|
| **Effort** | **M** (matches docs/research/finance; U1–U4 ~3–5 focused days if xfer sync deferred) |
...
- `docs/research/finance/features/multi-account-reconciliation.md`, `budgeting-envelopes.md`, `security/financial-data-security.md`, `01-feature-inventory.md`
