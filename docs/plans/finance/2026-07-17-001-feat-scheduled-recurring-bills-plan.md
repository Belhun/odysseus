...
priority: P1
benefit_verdict: yes
origin: docs/research/finance/features/recurring-bills.md
homebank_refs:
  - src/hb-archive.h
  - src/hb-archive.c
  - src/hub-scheduled.c
...
**Product Contract preservation:** Bootstrap from docs/research/finance + HomeBank archive model; no prior unified Product Contract.
...
**Authority:** This plan > `docs/research/finance/features/recurring-bills.md` for implementation shape; HomeBank is parity reference, not a line-by-line port.
...
2. **Unlocks Phase 2–3 research tracks.** Cash-flow forecast (`docs/research/finance/features/cash-flow-forecasting.md`), safe-to-spend, and subscription rollups all require confirmed recurring outflows as first-class data. Without templates, forecasts invent numbers from history alone.
...
|----|----------|-----------|
| **KTD1** | New `FinanceRecurring` model + `services/recurrence.py` | Matches docs/research/finance `finance_recurring`; isolates date math from routes/tools |
...
| `tests/test_finance_auto_post.py` | new |
| `docs/research/finance/features/recurring-bills.md` | optional status note after ship |
...
- docs/research/finance: `features/recurring-bills.md`, `subscription-tracking.md`, `cash-flow-forecasting.md`, `01-feature-inventory.md` (Phase 2 bills; Phase 3 CalDAV)
