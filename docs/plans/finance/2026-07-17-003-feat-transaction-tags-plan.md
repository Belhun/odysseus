...
origin: docs/research/finance/features/tags-labels.md
...
**Deferred:** tag groups/namespaces (`work:`), bulk tag from search results UI, HomeBank CSV tag import/export, rule `action_tags` (depends on rules expansion in `docs/research/finance/features/rules-auto-categorization.md`), tag merge UI, unused-tag cleanup job.
...
| `tests/test_finance_routes.py` | Extend |
| `tests/test_finance_agent_tools.py` | Extend |
| `docs/research/finance/features/tags-labels.md` | Optional: mark shipped sections |
...
| **Priority** | **High for agent usefulness; Medium for product roadmap.** Not in docs/research/finance MVP list (accounts/import/categories/budgets), but unblocks cross-cutting queries competitors treat as table stakes. Sequence after solid category/filter; before or alongside rules `action_tags`. |
...
- Odysseus: `docs/research/finance/features/tags-labels.md`, `search-filter-transactions.md`, `rules-auto-categorization.md`, `integrations/finance/models.py`, `routes.py`, `src/tools/finance.py`, `confirmation_gate.py`, `src/tool_schemas.py`
