...
This plan makes finance selection sticky within one chat, preserves the new batching contract, and adds compact server-side actions for rule overwrite and continuation, category-based classification, filter-based classification, remaining-work status, rule previews, and payee heuristics.
...
| Transaction listing | Batching work raised the agent cap to 200, added `offset`, `unclassified`, and `movement_class`, and prints class per row. | Keep the 25-row default and 200-row hard output cap, clamp invalid ranges, preserve totals, and strengthen paging/filter tests. |
...
| Class by category | No action maps category meaning to movement class. | Add `classify_by_category` with deterministic mapping, dry run, overwrite control, and a 500-row synchronous cap. |
...
| Search | `apply_transaction_filters()` applies one payee-only `ILIKE`. | Search payee or memo with punctuation-tolerant token matching; allow mutation callers to narrow `search_scope`. |
...
| Payee heuristics | `apply_payee_heuristics()` exists and is tested at service level, but is not an agent action. | Add a gated `apply_payee_heuristics` action with compact result counts. |
...
- R6. Agent listing must default to 25 rows and return at most 200 rows per call, with total count and the next offset when more rows exist.
...
- R13. `test_rule` must use the live payee-rule matcher and return total matches, fill-only eligible matches, overwrite changes, and at most 10 representative rows without mutating data.

**Server-side classification**

- R14. `classify_by_category` must infer `Family Transfers` as `pass_through`, `Credit Card Payments` and the existing Transfers label category as `transfer`, income categories as `income`, and other spend categories as `spend`; an explicit valid target class may override the inference.
- R15. Category classification must update only null classes by default and only null or different classes when `overwrite=true`.
...
- R18. Search terms must match payee or memo and tolerate punctuation between tokens, so `GOOGLE VOICE` can match `GOOGLE *VOICE`.
...
- R20. `apply_payee_heuristics` must expose the existing service behavior without overriding a non-null movement class.
...
- R22. `create_rule`, `apply_rule`, `classify_by_category`, `classify_by_filter`, `apply_payee_heuristics`, and all existing bulk mutations require confirmation unless Finance AI auto-approve is enabled.
- R23. Every agent-triggered synchronous set-based mutation must update at most 500 unique rows, report how many matching rows remain, and be safe to repeat.
...
- AE5. Covers R5-R6. A 3,347-row book returns 25 rows by default, never more than 200, states the full match count, and gives `offset=25` for the next page.
...
- AE10. Covers R14-R15. `classify_by_category` maps null-class Family Transfers rows to `pass_through`; a row already marked `spend` changes only when overwrite is true.
...
- AE13. Covers R20-R24. `apply_payee_heuristics` requires confirmation, honors auto-approve when enabled, updates at most 500 owner-scoped null-class rows, and returns compact changed and remaining counts.
...
- `test_rule`, `apply_rule`, `classify_by_category`, `classify_by_filter`, `classification_status`, and `apply_payee_heuristics`.
- Payee-or-memo tokenized search shared by HTTP listing and agent filters.
...
- KTD3. **Keep the agent read cap at 200 and the agent synchronous write cap at 500.** The HTTP list endpoint may return 500 for UI/API callers, but an agent response with 500 formatted rows is unnecessarily expensive. New set-based actions remove the need to list thousands of IDs before a write.
- KTD4. **Use one owner-scoped filter pipeline for list and set-based classification.** Extend `apply_transaction_filters()` with tokenized payee-or-memo search, `search_scope`, and `amount_sign`, then reuse it for dry-run counts and mutation candidate queries.
- KTD5. **Make agent set-based writes idempotent and bounded.** Default classification selects null targets. Overwrite selects null or different targets. Bulk payee expansion and heuristic application also skip already-satisfied rows. Each call takes the first deterministic 500 unique candidates across the whole action, updates them atomically, and reports the count still needing the target, so repeating the call makes progress without an offset race.
...
- KTD9. **Defer schema-heavy P2 work.** Existing columns are enough for movement-class rules, previews, status, and heuristics. Rule-condition columns, category defaults, and durable jobs each add migrations and wider UI/service contracts, so they remain separate follow-up work.
...
|---|---|---|---|---|
| `list_transactions` | Read | None | `account_id`, `category_id`, `month`, `start_date`, `end_date`, `min_amount_cents`, `max_amount_cents`, `search`, `search_scope`, `uncategorized`, `unclassified`, `movement_class`, `offset`, `limit` | At most 200 formatted rows, total, and next offset |
...
| `apply_payee_heuristics` | Write | None | `max_updates` | Compact changed count and remaining heuristic-eligible count |
...
- `search_scope`: `payee_or_memo` by default, with `payee` and `memo` as narrower options.
...
For payee heuristics, it must include `max_updates`.
Changing any of those values after approval invalidates the token.
...
9. After retrieval, domain seeding, model-specific tool clamps, and other tool composition, union allowlisted session pins before final prompt/schema filtering. Explicit disabled-tool, guide-only, plan-mode, plugin, and public-user policies still win.
10. Keep confirmation-token pinning intact as a separate one-turn safety path.
11. Do not modify `src/tool_index.py` `ALWAYS_AVAILABLE`, `ASSISTANT_ALWAYS_AVAILABLE`, or `_ADMIN_TOOLS`.
...
1. Keep `_MAX_TX_LIMIT=200`, `_MAX_BULK_TX=500`, the 25-row list default, and the current bulk action names.
2. Parse list pagination once: omitted `limit` becomes 25, numeric values below 1 become 1, values above 200 become 200, negative offsets become zero, and malformed non-integers return a validation error.
...
8. Build one deterministic, deduplicated candidate sequence across direct IDs and payee expansions. Skip rows already equal to every supplied target, update at most 500 unique rows across the whole call, and report matched, updated, and remaining counts so repeating the action progresses.
...
1. Add `infer_movement_class_for_category()` with ordered case-insensitive rules: exact Family Transfers leaf/path, exact Credit Card Payments leaf/path or `is_transfers_label_category()`, income, then spend.
2. Treat `FinanceCategory.is_income` and an Income-prefixed category path as income signals.
...
5. Preload the owner’s account-purpose map and derive each row’s stored target once, including `transfer` becoming `pass_through` on processor accounts, rather than issuing an account lookup per row.
6. For default mode, select null classes only.
7. For overwrite mode, select null or values different from that row’s resolved stored target so updated rows leave the remaining-work set.
...
- Family Transfers infers `pass_through`.
- Credit Card Payments infers `transfer`.
- Transfers (label only) retains the existing `transfer` meaning.
- A top-level or nested income category infers `income`.
- A normal expense category infers `spend`.
...
5. Default existing HTTP and agent list search to `payee_or_memo`.
...
7. Reject a write unless at least one meaningful narrowing filter is present; an invalid or empty tokenized search does not count. Account, category, date bound, amount bound, amount sign, or valid search are accepted narrowing fields.
...
### U6. Add tractable P2 rule, status, and heuristic actions

**Goal:** Finish the P2 items that fit existing storage and service boundaries.
...
1. Expose `movement_class` on the agent `create_rule` path; accept category, class, or both, using service-level validation so HTTP and agent callers reject the same invalid values.
2. Add `test_rule` over the matcher extracted in U3, with separate match and projected-change counts, processor-resolved class projections, and a 10-row sample.
3. Add `classification_status_counts()` using SQL aggregate queries rather than loading all rows or calling `review_queues()`, which materializes row details.
...
6. Extract the shared heuristic candidate/application logic and add an `apply_payee_heuristics_bounded()` service wrapper that tracks unique changed transaction IDs, reserves capacity for paired-row assignments, stops before exceeding `max_updates`, and returns changed plus remaining heuristic-eligible counts. Preserve the existing `apply_payee_heuristics()` integer return contract for current internal callers.
7. Add `apply_payee_heuristics` as a write action using the bounded wrapper.
8. Register `_validate_apply_payee_heuristics()` over `max_updates` and update auto-approve prompt guidance.
9. Keep test/status outputs compact and owner-scoped.
10. Do not add rule-condition or category-default columns.
...
- Existing `unclassified_counts()` and `apply_payee_heuristics()` behavior.
...
- Heuristics require confirmation when auto-approve is off.
- Heuristics run directly when auto-approve is on.
- Heuristics never replace a non-null class and preserve existing Zelle/Venmo safety cases from `tests/test_finance_movements.py`.
- Heuristics never change more than 500 unique rows, including paired assignments, and a repeated call advances remaining work.
- Heuristics response reports changed and heuristic-eligible remaining counts.
...
1. Update the finance domain rules and tool section with canonical action names and read/write distinctions.
...
4. Document the 200-row read cap, global 500-unique-row agent write cap, remaining-count loop, and exact semantics of `apply_to_payee`.
...
- Prompt text names the 200/500 caps and does not instruct per-row loops.
...
| Gate | Command | Proves |
|---|---|---|
| Session pinning | `pytest -q tests/test_tool_pins.py tests/test_agent_loop.py tests/test_chat_helpers.py tests/test_context_compactor.py` | Finance activation, chat isolation, allowlisting, off-topic behavior, full-history pin computation, and pin preservation across compaction |
...
| Movement services | `pytest -q tests/test_finance_movements.py` | Category/filter classification, 500-row bounds, processor resolution, and payee-heuristic safety |
...
- Confirm transaction samples are bounded at 10 and list output at 200.
- Confirm all agent mutation loops commit once and stop at 500 unique rows, including payee expansion, rule continuation, and paired heuristics.
...
- U2 is done when the current branch’s batching features remain intact, list output is bounded and pageable, payee expansion is exact and owner-scoped, conflicting groups fail atomically, and no agent batch updates more than 500 unique rows.
...
- U4 is done when category inference and bounded category classification work in dry-run and write modes.
...
- U6 is done when movement-class rules, rule preview, compact classification status, and bounded gated payee heuristics are available through `manage_finance`.
