...
ALWAYS_AVAILABLE = frozenset({
    ...
    "update_plan",
    # Search/unlock the full tool catalog when RAG selection missed a capability.
    "discover_tools",
})
...
ASSISTANT_ALWAYS_AVAILABLE = frozenset({
    "list_email_accounts", "list_emails", "read_local_emails", "sync_local_emails",
    "read_email", "send_email", "reply_to_email",
    "bulk_email", "archive_email", "delete_email", "mark_email_read",
    "manage_calendar", "manage_notes", "manage_tasks", "manage_finance",
    "manage_archive", "manage_dossier", "search_dossier",
...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "discover_tools": "Search or unlock tools when your current callable tool list is missing what you need. action=search with query describing the task (returns names + one-line descriptions, not full schemas). action=list with optional pattern to browse the catalog. action=unlock with tools=[names] to add tools for your NEXT agent round — then call the unlocked tool on the following round. MCP tools use qualified names mcp__server__tool. Do not substitute bash when a dedicated tool exists — discover and unlock it instead.",
    ...
    "read_local_emails": "Read emails from the local mirror store (instant, full history, works offline). List with limit/offset paging, search with q, or full=true with uid (preferred) or id for body + attachment paths. When Local only mode is on, this is the only read/search path.",
    "sync_local_emails": "Trigger a local email mirror sync (INBOX + Sent). Background task runs every 20 minutes when enabled; use for on-demand catch-up or full backfill.",
    ...
    "bulk_email": "Perform one action on many emails at once (live IMAP + local mirror). Use for delete all those, archive these, mark all read, move spam to junk. Takes explicit UIDs from list_emails/read_local_emails or all_unread=true. Always pass account for Gmail/work/custom mailbox results.",
    ...
    "manage_archive": "Ingest/get/list/search raw conversation archive items (messages, transcripts, pasted AI convos) with tool provenance (source_tool, locator). Prefer over manage_memory for other-person conversation evidence.",
    "manage_dossier": "People dossiers (friends/clients), situations, citeable plans with how-we-got-here context, key facts, timeline, and archive link propose/assign/correct. Prefer over manage_memory for person/situation plans and facts.",
    ...
    "manage_finance": "Local finance and budgeting: accounts, reports, pageable transaction search (payee or memo), classification status, categories, and rules. Prefer classification_status plus classify_by_category/classify_by_filter over paging IDs. list_transactions defaults to 25 and caps at 500. Writes cap at 500 unique rows, disclose match counts and unique payees, and reuse one logical confirmation for continuation pages. Preview rules before explicit overwrite; apply_to_payee is exact normalized equality. CSV/OFX import is UI-only.",
    ...
    "app_api": "Generic loopback to allowed Odysseus internal endpoints. Use this when the user wants something the UI can do but there's no named tool for it. Covers calendar, gallery, library/documents, memory, notes, tasks, settings, research, compare, cookbook GPUs/state, and Business Management (SysForge plugin) at /api/sysforge/* — clients, invoices, outstanding, merge, parts, projects, settings, diagnostics, drafts, payments, backup. Discover with action='endpoints' filter='sysforge'. Allowed UI buttons hit /api/* endpoints and you can hit them too. Sensitive auth/user/admin/shell paths and host-control Cookbook mutation routes are blocked; do NOT use app_api for shell commands, package installs, engine rebuilds, or PID signalling. Use named command tooling for shell commands. action='endpoints' with filter=<keyword> lists available endpoints. action='call' takes method+path+body (JSON only — no file uploads). Hits same routes the UI uses — auth flows free. NOTE: themes are NOT an API endpoint — use the ui_control tool (create_theme / set_theme), not app_api. SESSIONS/CHATS: do NOT use app_api for these — GET /api/sessions returns EMPTY for tool calls (it's owner-filtered and tool calls authenticate as a different identity). EMAIL ACCOUNTS: do NOT use /api/email/accounts via app_api; use list_email_accounts, list_emails, and read_email instead. To list/rename/archive/delete/fork chats use the list_sessions and manage_session tools instead.",
