...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            "name": "manage_finance",
            "description": "Read and manage local finance data. Prefer classification_status, test_rule, classify_by_category, and classify_by_filter over collecting or paging hundreds of transaction IDs. list_transactions defaults to 25 and caps at 500; agent search matches payee or memo. All synchronous writes cap at 500 unique rows and report remaining work. Preview match counts and unique payees before confirmation, then reuse one logical confirmation token for continuation pages. apply_to_payee uses exact normalized payee equality. Rule overwrite is false by default; preview collision-prone rules and prefer longer/specific patterns plus explicit priority.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {
                    ...
                    "confirmation_token": {
                        "type": "string",
                        "description": "Required for gated writes unless Finance AI auto-approve is enabled. Reuse it for continuation pages of the same payload-bound logical mutation.",
                    },
                    "transaction_id": {"type": "string", "description": "Single transaction id or prefix. Prefer transaction_ids when updating more than one row."},
                    "transaction_ids": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Many transaction ids or 8-char prefixes for classify_transaction / categorize_transaction. Max 500 per call. Do not loop one id at a time.",
                    },
                    "updates": {
                        "type": "array",
                        "description": "Mixed batch for bulk_update_transactions. Each item: transaction_ids plus movement_class and/or category_id.",
                        "items": {
                            "type": "object",
                            "properties": {
                                ...
                                "apply_to_payee": {"type": "boolean"},
                            },
                        },
                    },
                    "apply_to_payee": {
                        "type": "boolean",
                        "description": "Also apply the same class/category to other rows with the same payee",
                    },
                    "search": {"type": "string", "description": "Tokenized payee-or-memo search for agent list/classify filters"},
                    "search_scope": {"type": "string", "enum": ["payee", "memo", "payee_or_memo"], "description": "Search field scope; agent default payee_or_memo"},
                    "start_date": {"type": "string", "description": "YYYY-MM-DD start date filter"},
                    "end_date": {"type": "string", "description": "YYYY-MM-DD end date filter"},
                    "min_amount_cents": {"type": "integer", "description": "Minimum amount in cents for list_transactions"},
                    "max_amount_cents": {"type": "integer", "description": "Maximum amount in cents for list_transactions"},
                    "uncategorized": {"type": "boolean", "description": "Only uncategorized transactions for list_transactions"},
                    "unclassified": {"type": "boolean", "description": "Only rows with no movement_class for list_transactions"},
                    "offset": {"type": "integer", "description": "Skip this many rows for list_transactions paging"},
                    "limit": {"type": "integer", "description": "Max transactions to return (default 25, max 500)"},
                    ...
                    "pattern": {"type": "string", "description": "Payee regex/literal pattern for create_rule or test_rule; prefer longer/specific patterns"},
                    "priority": {"type": "integer", "description": "Rule priority for create_rule (lower number wins; default 100)"},
                    "apply_existing": {"type": "boolean", "description": "For create_rule: apply the new rule to existing matches (default true)"},
                    "rule_id": {"type": "string", "description": "Rule id or unique prefix for apply_rule"},
                    "overwrite": {"type": "boolean", "description": "Replace existing target fields only when explicitly true; default false"},
                    "dry_run": {"type": "boolean", "description": "Preview classify_by_category/filter counts and samples without confirmation or writes"},
                    "max_updates": {"type": "integer", "minimum": 1, "maximum": 500, "description": "Per-call mutation cap (default 500)"},
                    "amount_sign": {"type": "string", "enum": ["inflow", "outflow", "zero"], "description": "Signed amount filter for classify_by_filter"},
                    "date": {"type": "string", "description": "YYYY-MM-DD for create_transaction / update_transaction"},
                    "amount_cents": {"type": "integer", "description": "Signed amount in cents for create_transaction / update_transaction"},
                    "payee": {"type": "string", "description": "Payee for create_transaction / update_transaction"},
                    "memo": {"type": "string", "description": "Memo for create_transaction / update_transaction"},
                    "status": {"type": "string", "description": "Transaction status"},
                    "movement_class": {"type": "string", "description": "spend, income, transfer, pass_through, or reimbursement"},
                    "tx_ids": {"type": "array", "items": {"type": "string"}, "description": "Transaction ids for link_transactions, or alias of transaction_ids for bulk classify/categorize"},
                    "posted_pin_cents": {"type": "integer", "description": "Posted pin for pin_account"},
                    "available_cents": {"type": "integer", "description": "Available snapshot for pin_account"},
                    "clear_posted_pin": {"type": "boolean", "description": "Clear the posted pin"},
                    "clear_available": {"type": "boolean", "description": "Clear the available snapshot"},
                },
                "required": ["action"],
            },
        },
