...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            "name": "manage_finance",
            "description": "Read and manage local finance data: accounts, transactions, spending by category, budgets, trends, and net worth. Use for spending questions, budget checks, and transaction lookups. Bank CSV/OFX import is UI-only — use ui_control open_panel finance to open the Import tab. Prefer spending_report for 'where did my money go' questions; use list_transactions only when the user needs specific rows (max 50). Mutations (create_category, create_categories, categorize_transaction, set_budget, create_rule) normally need ask_user confirmation first unless the user enabled Finance AI auto-approve in Settings. For multiple categories, put an `items` array in confirmation.payload (or use create_categories with a `categories` array after approval). Reuse the same confirmation_token for each create_category call, or create them all at once with create_categories. Use suggest_categories for uncategorized payee groups; then create_rule after user approval.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {
                    ...
                    "confirmation_token": {
                        "type": "string",
                        "description": "Required for gated actions (create_category, create_categories, set_budget, create_rule, categorize_transaction) — token from ask_user after user approval",
                    },
                    ...
                    "tx_ids": {"type": "array", "items": {"type": "string"}, "description": "Transaction ids for link_transactions"},
