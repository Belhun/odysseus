    ...
    <div class="modal-content doclib-modal-content" style="width:min(720px, 92vw);background:var(--bg);">
      <div class="modal-header">
        ...
        <div class="email-lib-header-actions" style="display:flex;align-items:center;gap:8px;">
          <label class="admin-switch email-local-only-switch" title="List and read from the local mirror instead of live IMAP">
            <input type="checkbox" id="email-lib-local-only">
            <span class="admin-slider"></span>
          </label>
          <span id="email-lib-local-only-label" style="font-size:11px;opacity:0.75;user-select:none;">Local only</span>
          <span id="email-lib-sync-chip" class="memory-count" style="font-size:10px;opacity:0.55;display:none;"></span>
...
async function _doSearch() {
  ...
  try {
    ...
    const res = await emailApi.searchEmails(q, {
      folder: folderAtStart,
      limit: 100,
      accountId: accountAtStart,
    });
    const data = await res.json();
    if (
      seq !== _libSearchSeq ||
...
async function _updateLocalSyncChip() {
  const chip = document.getElementById('email-lib-sync-chip');
  if (!chip) return;
  const localOnly = await emailApi.loadLocalOnlyMode();
  if (!localOnly) {
    chip.style.display = 'none';
    return;
  }
  try {
    const status = await emailApi.loadLocalModeStatus();
    const ts = status?.last_sync_at;
    chip.textContent = ts ? `synced ${ts.replace('T', ' ').slice(0, 16)}` : 'not synced yet';
    chip.style.display = '';
  } catch (_) {
    chip.style.display = 'none';
  }
...
function _wireLocalOnlyToggle() {
  const toggle = document.getElementById('email-lib-local-only');
  if (!toggle || toggle.dataset.wired) return;
  toggle.dataset.wired = '1';
  emailApi.loadLocalOnlyMode().then((on) => {
    toggle.checked = !!on;
    _updateLocalSyncChip();
  }).catch(() => {});
  toggle.addEventListener('change', async () => {
    try {
      await emailApi.setLocalOnlyMode(toggle.checked);
...
async function _refreshUnreadBadge() {
  ...
  try {
    ...
    if (state._libFilter === 'unread') {
      ...
      try {
        const allRes = await emailApi.listEmails({
          folder,
          limit: 1,
          filter: 'all',
          accountId: window.__odysseusActiveEmailAccount || '',
  ...
  try {
    ...
    if (folderAtStart === '__scheduled__') {
      ...
      const res = await emailApi.listEmails({
        ...
        accountId: accountAtStart,
        hasAttachments: hasAttachmentsAtStart,
        buster: force ? `&_=${Date.now()}` : '',
      });
      const data = await res.json();
...
async function _toggleCardPreview(card, em) {
  ...
  try {
    const res = await emailApi.readEmail(em.uid, {
      folder: folderAtStart,
      accountId: accountAtStart,
    });
    const data = await res.json();
...
async function _toggleFromSenderPanel(reader, data, btn) {
  ...
  try {
    ...
    const res = await emailApi.searchEmails(fromAddr, {
...
async function _swapReaderToUid(reader, uid, folder) {
  ...
  const useFolder = folder || state._libFolder;
  try {
    const res = await emailApi.readEmail(uid, {
      folder: useFolder,
      accountId: state._libAccountId || '',
    });
    const data = await res.json();
