...
async function _refreshUnreadCount() {
  ...
  try {
    ...
    const [listRes, urgRes] = await Promise.all([
      emailApi.listEmails({ folder: 'INBOX', limit: 50, filter: 'unread', accountId: window.__odysseusActiveEmailAccount || '' }),
...
export function markInboxAsSeen() {
  ...
  try {
    ...
    emailApi.listEmails({ folder: 'INBOX', limit: 1, accountId: window.__odysseusActiveEmailAccount || '' })
      .then(r => r.json())
      .then(data => {
        const emails = data.emails || [];
        if (emails.length > 0) {
...
export async function loadEmails(append = false) {
  ...
  try {
    ...
    if (!append) _emails = [];
    let batch = data.emails || [];
    if (_senderFilter) {
      const target = _senderFilter.toLowerCase();
      batch = batch.filter(e => String(e.from_address || '').toLowerCase().includes(target));
    }
    _emails.push(...batch);
    _total = data.total || 0;
