...
def _iter_attachment_parts(msg):
    ...
    for part in msg.walk():
        ...
        is_attached_email = ct == "message/rfc822" and ("attachment" in cd or part.get_filename())
        ...
        if payload is None:
            payload = b""
        yield idx, part, filename, payload
        idx += 1


def _list_attachments_from_msg(msg, *, include_payload: bool = False):
    """Return a list of attachment metadata from an email message."""
    attachments = []
    for idx, _part, filename, payload in _iter_attachment_parts(msg):
        meta = {
            "index": idx,
            "filename": filename,
            "content_type": _part.get_content_type(),
            "size": len(payload),
            "is_inline": "inline" in str(_part.get("Content-Disposition", "")).lower(),
        }
        if include_payload:
