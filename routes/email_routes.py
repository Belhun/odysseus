...
from routes.email_helpers import (
    ...
    _group_uid_fetch_records, _uid_from_fetch_meta,
...
def setup_email_routes():
    ...
    from routes.email_local_store import (
        ...
        get_sync_status,
        normalize_local_list_row,
        search_local_emails,
    )
    from src.settings import get_user_setting

    @router.get("/local/mode")
    async def local_email_mode(owner: str = Depends(require_owner)):
        sync_state = await asyncio.to_thread(get_sync_status, owner)
        last_sync_at = None
        for row in sync_state:
            ts = row.get("last_sync_at")
            if ts and (last_sync_at is None or ts > last_sync_at):
                last_sync_at = ts
        return {
            "local_only": bool(get_user_setting("email_local_only", owner, False)),
    ...
    async def list_local_emails(
        ...
        if account_id:
            _assert_owns_account(account_id, owner)
        local_filter = filter
        if local_filter == "favorites":
            local_filter = "flagged"
        rows, total, resolved_folder = await asyncio.to_thread(
            query_local_emails,
            owner,
            ...
            since=since,
            until=until,
            filter_=local_filter if local_filter in {"all", "unread", "unanswered", "flagged"} else "all",
            has_attachments=has_attachments,
            include_total=True,
        )
        emails = [
            normalize_local_list_row(r, resolved_folder)
            for r in rows
        ]
        return {"emails": emails, "total": total, "folder": resolved_folder, "offset": offset}

    @router.get("/local/search")
    async def search_local_email_store(
        q: str = Query(""),
        folder: str = Query("INBOX"),
        ...
        if not q or len(q.strip()) < 2:
            return {"emails": [], "total": 0, "query": q, "folder": folder, "offset": offset}
    ...
    @router.get("/local/read/{uid}")
    async def read_local_email_by_uid(
        uid: str,
        folder: str = Query("INBOX"),
        account_id: str | None = Query(None),
        owner: str = Depends(require_owner),
